...
public class RequestFTPResource {
	...
	private String corps() {
		String css = "<style type=\"text/css\">"
				...
				+ "</style>";
		String head = "<div id='home'><div id='related_links'>" + FTP_SERVER
				+ "<a href=" + BASE_URL + "cdup>cdup</a>"
				+ "<br/><a href=\"upload\">Charger un fichier</a><p>" + "path:"
				+ this.ftp.pwd().substring(3) + "</p>" + "<ul>";
		String corps = "";
		String[] ls = this.ftp.ls().split(",,");
		for (String n : ls) {
			String[] tmp = n.split(" ");
			if (tmp.length == 2 && tmp[1].equals("-dir-"))
				corps = corps + "<li>" + "<a href=" + BASE_URL + "cd/" + tmp[0]
						+ ">" + tmp[0] + "/</a>" + "</li>";
			else
				corps = corps + "<li>" + "<a href=" + BASE_URL + "get/"
						+ tmp[0] + ">" + tmp[0] + "</a>" + "</li>";
		}
		if (corps.equals("")) {
	...
	public Response get(@PathParam("fichier") String fichier) {
		return javax.ws.rs.core.Response.ok(this.ftp.get(fichier),
				javax.ws.rs.core.MediaType.APPLICATION_OCTET_STREAM).build();
	}
